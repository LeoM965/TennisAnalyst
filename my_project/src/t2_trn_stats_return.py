...
from t2_trn_stats_return_indicators import calculate_return_indicators
from constants import WTA_MCP_RETURN, RETURN_INDICATORS

def analyze_return_data(csv_path=WTA_MCP_RETURN):
    output_directory = 'output_return'
    ...
    dataframe_with_indicators = calculate_return_indicators(dataframe)
    
    if 'Overall_RiP' in dataframe_with_indicators.columns:
        main_aggregation_key = 'Overall_RiP'
    else:
        main_aggregation_key = 'Return_In_Play_Rate'
    ...
    yearly_stats_path = os.path.join(output_directory, 'yearly_return_stats.csv')
    ...
    career_stats_path = os.path.join(output_directory, 'career_return_stats.csv')
    career_stats.to_csv(career_stats_path, index=False)
    
    match_view_columns = ['Player', 'Match', 'Result', 'Year'] + [
        col for col in RETURN_INDICATORS if col in dataframe_with_indicators.columns
    ]
    match_stats = dataframe_with_indicators[match_view_columns].round(3)
    
    match_stats_path = os.path.join(output_directory, 'match_return_stats.csv')
    match_stats.to_csv(match_stats_path, index=False)
    
    available_metrics = [col for col in RETURN_INDICATORS if col in career_stats.columns]
    ...
    if available_metrics:
        ...
        if top_performers_collection:
            ...
            top_performers_path = os.path.join(output_directory, 'top_return_performers.csv')
