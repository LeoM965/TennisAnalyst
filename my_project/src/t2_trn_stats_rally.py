...
def analyze_rally_data(csv_path=WTA_MCP_RALLY):
    ...
    dataframe_with_indicators = calculate_rally_indicators(dataframe)
    
    aggregation_rules = {
        'RallyLen': ['mean', 'count']
    }
    
    for indicator in RALLY_INDICATORS:
    ...
    yearly_stats_path = os.path.join(output_directory, 'yearly_rally_stats.csv')
    ...
    career_stats_path = os.path.join(output_directory, 'career_rally_stats.csv')
    career_stats.to_csv(career_stats_path, index=False)
    
    match_view_columns = ['Player', 'Match', 'Result', 'Year'] + [
        col for col in RALLY_INDICATORS if col in dataframe_with_indicators.columns
    ]
    match_stats = dataframe_with_indicators[match_view_columns].round(3)
    
    match_stats_path = os.path.join(output_directory, 'match_rally_stats.csv')
    match_stats.to_csv(match_stats_path, index=False)
    
    available_metrics = [col for col in RALLY_INDICATORS if col in career_stats.columns]
    ...
    if available_metrics:
        ...
        if top_performers_collection:
            ...
            top_performers_path = os.path.join(output_directory, 'top_rally_performers.csv')
